...
import ru.otus.spring.hw02.config.Formatter;
...
public class QuestionFormatterImpl implements QuestionFormatter {
    ...
    private String answerToString(Answer answer, int answerNumber) {
        var answerNumberPrefix
                = appProperties.getFormatter().answerArabicNumerationEnable()
                ? String.format("%d. ", answerNumber + 1) : "";
        return String.format("%s%s%s%n",
                emptyIfNull(appProperties.getFormatter().answerTabulation()), answerNumberPrefix, answer.getText());
    }
}
